...
if ($search) {
	...
	if (($_REQUEST['query'] && strlen($_REQUEST['query']) < 3) || (!$_REQUEST['query'] && isset($_REQUEST['basic_search']))) {//Why do I care about this crap...
		$search = false;
		//Instead of an error page...default back to regular query..with no search.
...
$staffId = 0;
//Nothing for now...TODO: Allow admin and manager to limit tickets to single staff level.
$showassigned = true;
//show Assigned To column - defaults to true
...
$qwhere = ' WHERE ( ' . '  ( ticket.staff_id=' . db_input($thisstaff -> getId()) . ' AND ticket.status="open")';

if (!$thisstaff -> showAssignedOnly())
	$qwhere .= ' OR ticket.dept_id IN (' . ($depts ? implode(',', db_input($depts)) : 0) . ')';

if (($teams = $thisstaff -> getTeams()) && count(array_filter($teams)))
	$qwhere .= ' OR (ticket.team_id IN (' . implode(',', db_input(array_filter($teams))) . ') AND ticket.status="open")';
...
if (isset($_REQUEST['uid']) && $_REQUEST['uid']) {
	$qwhere .= ' AND (ticket.user_id=' . db_input($_REQUEST['uid']) . ' OR collab.user_id=' . db_input($_REQUEST['uid']) . ') ';
...
if ($staffId && ($staffId == $thisstaff -> getId())) {//My tickets
	...
	if (!($cfg -> showAssignedTickets() || $thisstaff -> showAssignedTickets())) {
		...
		//XXX: NOT factoring in team assignments - only staff assignments.
		$showassigned = false;
		//Not showing Assigned To column since assigned tickets are not part of open queue
	}
}
...
if ($search) :
	...
	if ($searchTerm) {
		...
		if (is_numeric($searchTerm)) {
			...
			if (count($tickets))
				$qwhere .= ' AND ticket.ticket_id IN (' . implode(',', db_input($tickets)) . ')';
			else
...
if ($_REQUEST['advsid'] && isset($_SESSION['adv_' . $_REQUEST['advsid']])) {
	...
	$qwhere .= ' AND ticket.ticket_id IN (' . implode(',', db_input($_SESSION['adv_' . $_REQUEST['advsid']])) . ')';
}

$sortOptions = array('date' => 'effective_date', 'ID' => 'ticket.`number`*1', 'pri' => 'pri.priority_urgency', 'name' => 'user.name', 'subj' => 'cdata.subject', 'status' => 'ticket.status', 'assignee' => 'assigned', 'staff' => 'staff', 'dept' => 'dept.dept_name');
...
if (!$order_by) {
	if ($showanswered)
		$order_by = 'ticket.lastresponse, ticket.created';
	//No priority sorting for answered tickets.
	elseif (!strcasecmp($status, 'closed'))
		$order_by = 'ticket.closed, ticket.created';
	//No priority sorting for closed tickets.
...
$sort = $_REQUEST['sort'] ? strtolower($_REQUEST['sort']) : 'pri.priority_urgency';
//Urgency is not on display table.
$x = $sort . '_sort';
...
$qselect = 'SELECT ticket.ticket_id,tlock.lock_id,ticket.`number`,ticket.dept_id,ticket.staff_id,ticket.team_id ' . ' ,user.name' . ' ,email.address as email, dept.dept_name ' . ' ,ticket.status,ticket.source,ticket.isoverdue,ticket.isanswered,ticket.created ';

$qfrom = ' FROM ' . TICKET_TABLE . ' ticket ' . ' LEFT JOIN ' . USER_TABLE . ' user ON user.id = ticket.user_id' . ' LEFT JOIN ' . USER_EMAIL_TABLE . ' email ON user.id = email.user_id' . ' LEFT JOIN ' . DEPT_TABLE . ' dept ON ticket.dept_id=dept.dept_id ';
...
<?php csrf_token(); ?>
     ...
     <tbody>
        ...
        if($res && ($num=count($results))):
            ...
            <tr id="<?php echo $row['ticket_id']; ?>">
                ...
                <td nowrap>&nbsp;<?php echo Format::htmlchars(Format::truncate($row['name'], 22, strpos($row['name'], '@'))); ?>&nbsp;</td>
                <?php
                if($search && !$status){
 ...
 ?>
    ...
    if($num>0){ //if we actually had any tickets returned.
        ...
        echo sprintf('<a class="export-csv" href="?%s">Export</a></div>',
    ...
    <form action="tickets.php" method="post" id="search" name="search">
        ...
        foreach (TicketForm::getInstance()->getFields() as $f) {
            ...
            ?><label><?php echo $f -> getLabel(); ?>:</label>
                <div style="display:inline-block;width: 12.5em;"><?php $f -> render('search'); ?><
/div><?php } ?>
</fieldset>
<p>
	<span class="buttons">
	...
	<span class="spinner"> <img src="./images/ajax-loader.gif" width="16" height="16"> </span>
</p>
</form>
